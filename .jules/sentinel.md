...
## 2026-08-25 - Prevent Secret Leaks in Tool Error Messages
**Vulnerability:** When explicit tool handlers like `notion_brain_task` and `notion_brain_search` failed (either due to API errors throwing `RuntimeError` or invalid status/database string input formatting errors), the unredacted exception message or user string was returned directly. This could lead to a leakage of API tokens and secrets embedded in input or underlying network messages.
**Learning:** Security measures such as `S.redact_secrets()` must be actively applied to all user-facing output messages and errors, including those that are just passing along exception messages, as exceptions may encapsulate or log secrets.
**Prevention:** Wrap string representations of all raised errors, as well as inputs reflected in string-based formatting errors, with the `redact_secrets()` sanitization mechanism before returning them to the client.
